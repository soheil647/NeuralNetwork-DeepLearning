...
class cifar10vgg:
    # melika: change train from true to false
    ...
    #
    # def predictsubmodel(self,x,submodel,normalize=True,batch_size=50):
    #     if normalize:
    #         x = self.normalize_production(x)
    #     return submodel.predict(x,batch_size)
...
if __name__ == '__main__':
    ...
    output_file_train = open('cifar10_train.txt', 'a+', 1)
    output_file_test = open('cifar10_test.txt', 'a+', 1)
    for i in range(0, model.get_length_model()):
        ...
        # prev = time.time()
        result2, number2 = calculate_SI_CPU(x_test_out, y_test_one_hot)
        ...
        result1, number1 = calculate_SI_CPU(x_train_out, y_train_one_hot)
